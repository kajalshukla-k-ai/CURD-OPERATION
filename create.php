...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name_err) && empty($email_err) && empty($mobile_err)&& empty($dob_err) && empty($pincode_err)){
        ...
        $sql = "INSERT INTO data (name, email, mobile,dob,pincode) VALUES (?, ?, ?, ?,?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssss", $param_name, $param_email, $param_mobile, $param_dob, $param_pincode);
